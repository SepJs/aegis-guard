// phase5_bridge.rs — Phase 5: behavioral, threat-intel, self-protect, canary
...
pub async fn start(
    ...
    match ThreatMatcher::new() {
        Ok(matcher) => {
            info!("threat intel loaded: {:?}", matcher.stats());
            matcher.start_auto_update();
            info!("threat intel auto-update started (6h interval)");
        }
    ...
    tokio::spawn(async move {
        while let Some(ev) = sp_rx.recv().await {
            ...
            let incident = ThreatIncident {
                ...
                ancestors:  vec![],
                ts:         Utc.timestamp_millis_opt(ev.ts).unwrap(),
                resolved:   false,
                digest:     String::new(),
            };
        ...
        for pid in pids {
            ...
            if let Some(anomaly) = engine.observe(obs) {
                ...
                let incident = ThreatIncident {
                    ...
                    ancestors:  vec![],
                    ts:         Utc.timestamp_millis_opt(anomaly.ts).unwrap(),
                    resolved:   false,
                    digest:     String::new(),
                };
