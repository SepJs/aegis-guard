// DrillDownModal.tsx — Phase 4: includes Kill / Quarantine / Whitelist actions
...
export default function DrillDownModal({ node, onClose }: Props) {
  ...
  // Convert ProcessNode → ThreatIncident shape for ConfirmModal
    ...
    <>
      <div className="modal-backdrop" onClick={onClose}>
        <div className="modal" onClick={e => e.stopPropagation()}>
          ...
          {successMsg && (
            <div style={{
              margin: "0 16px 0", padding: "8px 12px",
              background: "rgba(13,148,136,.1)", border: "1px solid var(--teal)",
              borderRadius: 4, fontSize: 10, color: "var(--teall)",
            }}>
              ✓ {successMsg}
            </div>
          )}
          ...
          <div className="modal-section">
            <div className="modal-section-title">RESPONSE ACTIONS — PHASE 4</div>
            <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap" }}>
              <button
                className="response-btn response-btn--kill"
                onClick={() => setActiveAction("kill")}
                title="Send SIGTERM → 5s grace → SIGKILL. IRREVERSIBLE."
              >
                ✕ KILL
              </button>
              <button
              ...
              <button
                ...
                title="Add to allowlist — never flagged again."
              >
                ✓ WHITELIST
              </button>
            </div>
            <p style={{ fontSize: 9, color: "var(--tx2)", marginTop: 6, fontStyle: "italic" }}>
      ...
      {/* Confirm modal — shown when user clicks a response action */}
      {activeAction && (
        <ConfirmModal
          incident={asIncident}
          action={activeAction}
