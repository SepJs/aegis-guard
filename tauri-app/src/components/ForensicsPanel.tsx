...
function fmtBytes(n: number) { if (n < 1024) return `${n} B`; if (n < 1024*1024) return `${(n/1024).toFixed(1)} KB`; return `${(n/1024/1024).toFixed(1)} MB`; }
...
function EntropyBar({ value }: { value: number }) {
  ...
  return <div className="entropy-track"><div className={`entropy-fill ${cls}`} style={{ width: `${pct.toFixed(1)}%` }} /></div>;
}
...
function ResultRow({ r }: { r: FileScanResult }) {
  ...
  return (
    <>
      ...
      {open && <div className="result-detail"><div className="meta-row"><span className="meta-label">PATH</span><code className="meta-code">{r.path}</code></div><div className="meta-row"><span className="meta-label">NOTE</span><span className="meta-val">{r.note}</span></div></div>}
    </>
  );
}
...
export default function ForensicsPanel() {
  ...
  const runScan = useCallback(async (path: string) => {
    ...
    try { const r = await invoke<ScanSummary>("scan_entropy", { request: { path, max_bytes: 33554432, recursive: true } }); setSummary(r); }
  ...
  return (
    <div className="panel">
      <div className="panel-header">
        ...
        {summary && <><span className="pstat">{summary.scanned_files} SCANNED</span>{summary.high_risk > 0 && <span className="pstat pstat--warn">⚠ {summary.high_risk} HIGH-RISK</span>}<span className="scan-time">{summary.elapsed_ms}ms</span></>}
        <div className="toolbar-right">
          {summary && <select className="filter-select" value={filter} onChange={e => setFilter(e.target.value)}>
            <option value="all">ALL RESULTS</option><option value="high">HIGH RISK</option><option value="medium">MEDIUM RISK</option><option value="normal">NORMAL</option><option value="skipped">SKIPPED</option>
          </select>}
          <input ref={inputRef} type="file" style={{ display: "none" }} onChange={e => { const f = e.target.files?.[0]; if (f) runScan((f as any).path ?? f.name); }} />
          <button className="sm-btn" onClick={() => inputRef.current?.click()} disabled={scanning}>BROWSE FILE…</button>
          {summary && <button className="sm-btn" onClick={() => { setSummary(null); setFilter("all"); }}>NEW SCAN</button>}
        </div>
      </div>
      ...
      {!scanning && !summary && !error && (
        <div className={`drop-zone ${dragOver ? "drop-zone--active" : ""}`}
          ...
          onDrop={e => { e.preventDefault(); setDragOver(false); const f = e.dataTransfer.files[0]; if (f) runScan((f as any).path ?? f.name); }}
          onClick={() => inputRef.current?.click()}>
          <div className="drop-icon-box">+</div>
          <div className="drop-title">DROP FILE OR DIRECTORY HERE</div>
          <div className="drop-sub">Shannon entropy analysis — packed binaries and encrypted blobs score near 8.0 bits/byte. No files are modified.</div>
          <div className="drop-sub drop-passive">PASSIVE ANALYSIS ONLY</div>
        </div>
      )}
      {scanning && <div className="empty-state"><div className="spinner" /><span>SCANNING…</span></div>}
      {error && <div className="empty-state" style={{ color: "var(--redl)" }}><span className="empty-icon" style={{ color: "var(--red)" }}>✕</span>{error}</div>}
      {summary && !scanning && (
        <>
          ...
          <div className="result-list">{displayed.length === 0 ? <div className="empty-state">NO RESULTS MATCH FILTER</div> : displayed.map((r, i) => <ResultRow key={i} r={r} />)}</div>
        </>
      )}
    </div>
  );
