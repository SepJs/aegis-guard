...
export default function CanaryPanel() {
  ...
  async function load() { try { setTokens(await invoke<CanaryToken[]>("list_canaries")); } catch (e) { console.error(e); } }
  useEffect(() => { load(); }, []);

  async function create() {
    if (!filePath.trim()) return;
    setCreating(true); setError(null);
    try { await invoke("create_canary", { filePath: filePath.trim(), description: desc.trim() }); setFilePath(""); setDesc(""); await load(); }
    catch (e) { setError(String(e)); } finally { setCreating(false); }
  }

  async function remove(id: string) { try { await invoke("delete_canary", { id }); await load(); } catch (e) { console.error(e); } }
  ...
  return (
    <div className="panel">
      <div className="panel-header">
        ...
        {tokens.some(t => t.triggered) && <span className="pstat pstat--warn">⚠ {tokens.filter(t => t.triggered).length} TRIGGERED</span>}
      </div>
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)", display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <button className="action-btn" style={{ width: "fit-content" }} onClick={create} disabled={creating || !filePath.trim()}>{creating ? "CREATING…" : "CREATE CANARY TOKEN"}</button>
      </div>

      <div style={{ flex: 1, overflowY: "auto" }}>
        {tokens.length === 0 ? <div className="empty-state"><span className="empty-icon">⬟</span>NO CANARY TOKENS — create one above</div> : tokens.map(t => (
          <div key={t.id} style={{ padding: "10px 16px", borderBottom: "1px solid var(--border)", borderLeft: `2px solid ${t.triggered ? "var(--red)" : "var(--teal)"}`, display: "flex", flexDirection: "column", gap: 5 }}>
