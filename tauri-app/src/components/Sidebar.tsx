...
export type View =
  | "processes" | "journal" | "forensics"
  | "debug"     | "audit"   | "intel" | "canary";

interface Props { active: View; setActive: Dispatch<SetStateAction<View>>; }
        ...
        {NAV.map(({ id, icon, label }) => (
          <li key={id}>
            ...
            >
              ...
              {id === "journal" && store.openCount > 0 && (
                <span className="nav-badge">{store.openCount}</span>
              )}
            </div>
          </li>
        ))}
      ...
      <div className="sidebar-footer">
        <div className={`engine-status ${connected ? "status--live" : "status--wait"}`}>
          ...
          <span className="status-label">
            {connected
              ? `ENGINE LIVE · ${store.nodes.size} PROCS`
              : "WAITING FOR ENGINE…"}
          </span>
        </div>
        <div className="phase-tag">PHASE 5 · FULL STACK · LINUX</div>
        <UpdateBanner />
      </div>
    </nav>
  );
