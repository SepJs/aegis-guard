...
export default function ThreatJournal() {
  ...
  ["PAR","PATH","ARG","ENV"].forEach(c => {
    counts[c] = incidents.filter(i => i.rule.startsWith(c)).length;
  });
  ...
  return (
    <div className="panel">
      <div className="panel-header">
        ...
        {open.length > 0 && (
          <span className="pstat pstat--warn">{open.length} OPEN</span>
        )}
...
function IncCard({ inc, expanded, onToggle, onResolve }: {
  inc: ThreatIncident; expanded: boolean;
      ...
      {expanded && (
        <div className="card-detail">
          ...
          <div className="meta-row">
            ...
            <span className="meta-val">{inc.kind.replace(/_/g, " ").toUpperCase()}</span>
          </div>
          <div className="meta-row">
            <span className="meta-label">PID / PPID</span>
            <span className="meta-val">{inc.pid} / {inc.ppid}</span>
          </div>
