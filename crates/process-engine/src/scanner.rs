// scanner.rs — /proc poller (Phase 2: runs both PAR + PATH rule engines)
...
async fn run_scanner(
    ...
    loop {
        ...
        for (pid, info) in &current {
            ...
            // Phase 1: parentage rules
            let anomaly = rules.evaluate(info, &known)
                // Phase 2: path/arg/env rules (only if PAR found nothing)
                .or_else(|| path_rules.evaluate(info, &known));

            let kind = if anomaly.is_some() {
                ProcEventKind::Anomaly
            } else {
                ProcEventKind::Spawned
            };
...
fn read_proc_info(pid: u32) -> Option<ProcInfo> {
    ...
    let exe         = fs::read_link(format!("{}/exe", base)).ok()
        .map(|p| p.to_string_lossy().into_owned());
    let cwd         = fs::read_link(format!("{}/cwd", base)).ok()
        .map(|p| p.to_string_lossy().into_owned());
