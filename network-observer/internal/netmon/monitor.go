...
//
// Reads /proc/net/{tcp,tcp6,udp,udp6} directly — no external Go modules needed.
// This gives us real-time TCP/UDP connection lifecycle without any kernel
// modules, eBPF, or root-level packet capture.
//
// Strategy:
...
func (m *Monitor) emit(conn ProcessConn) {
	...
	// Run network rules — convert to the rules package's input type
	connInfo := &rules.ConnInfo{
		PID:        conn.PID,
		Process:    conn.Process,
		UID:        conn.UID,
	...
	if alert := m.rules.Evaluate(connInfo); alert != nil {
		ev.Kind  = "net_alert"
		ev.Alert = alert
	}
