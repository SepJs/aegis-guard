...
import ("context"; "fmt"; "log/slog"; "os"; "os/signal"; "syscall"
...
func main() {
	...
	slog.Info("Aegis-Guard Network Observer starting", "phase", "3", "rules", "NET-001..005", "bridge", "127.0.0.1:50053")
	...
	go func() {
		for ev := range eventCh {
			if ev.Alert != nil {
				slog.Warn("NETWORK ANOMALY", "rule", ev.Alert.Rule, "confidence", ev.Alert.Confidence, "process", ev.Process, "pid", ev.PID, "remote", fmt.Sprintf("%s:%d", ev.RemoteIP, ev.RemotePort), "reason", ev.Alert.Reason)
			}
			select { case bridgeCh <- ev: default: }
		}
	}()
...
func logLevel() slog.Level {
	...
	case "debug", "trace": return slog.LevelDebug
